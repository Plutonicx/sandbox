import cv2
from get_data import get_data, check_size
import numpy as np
import pandas as pd
...
# val_batch = minibatchAB(horses, zebras, batch_size=4)
# _, val_A, val_B = next(val_batch)
# train_batch = minibatchAB(horses, zebras, batch_size)

horses = np.array(horses)
zebras = np.array(zebras)
...
while epoch_count < how_many_epochs:
    ...
    A = horses[
        np.random.randint(0,horses.shape[0], size=batch_size),
        :,
        :,
        :
    ]
    ...
    B = zebras[
        np.random.randint(0,zebras.shape[0], size=batch_size),
        :,
        :,
        :
    ]
